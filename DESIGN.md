...
**판정 기준**: 배경 교란 시 낙폭이 작고 실내 배경 분할에서 성능이 유지되는 전처리를 채택한다.
원본(full) 정확도가 더 높게 나오더라도, 배경 교란에 취약하면 채택하지 않는다.
이 기준을 **실험 전에 고정**해 사후 선택 논란을 차단한다.
...
3. **라이선스 정책 결정**: ND/ARR 이미지를 *학습*에 쓸 것인가.
   학습 사용과 재배포는 별개 문제이나, 보수적으로 가려면 ND 34장을 학습에서도 제외하는 선택지가 있다.
   제외 시 영향이 미미하므로(전체의 1.7%) **ND는 제외를 권장**한다. ARR은 학습에만 사용하고 배포하지 않는다.
   어느 쪽이든 논문 Dataset 절에 명시한다.
...
| null 결과 사전 규정 | SX는 test에서 역전됐고 form head는 기준선 미달이다. **E5가 또 null이면 그것을 어떻게 보고할지 지금 정해 둔다.** 사후에 정하면 p-hacking으로 읽힌다 |
